...
| `src/main/resources/rdt_quantities.json` | Stack size of the rare/gem-table roll, for items a monster also drops itself (generated) — see "Same item on both tables" |
...
## Regenerating data
`droprates_clean.json`, `rare_drop_table.json`, `npc_versions.json` and `rdt_quantities.json` are generated by
...
## Same item on both tables: the stack size decides (fixed 2026-09-20, issue #15)

Sarachnis drops 20–30 noted Uncut sapphire from its own table at `4/100` and a single
one from the gem drop table at `1/400`. "The monster's own table always beats the RDT"
(below) is right for Araxxor's 2 noted Rune kiteshields, and wrong for that lone
sapphire: it printed `1/25` for a 1/400 drop. Quantity is the only discriminator.

- `tools/crawl_bucket.py` writes `rdt_quantities.json` — `{monster: {item: "N" | "LOW-HIGH"}}`,
  the rare/gem table's quantity from the wiki rows' `Quantity Low` / `Quantity High`.
  A pair is emitted **only** when that quantity overlaps none of the monster's own rows
  for the item (any row with `Varies`/`Unknown` counts as overlapping). 133 pairs across
  62 monsters on 2026-09-20: KBD, Kalphite Queen, Giant Mole, Mithril dragon, ...
- `resolveDrop` takes the stack quantity. A stack inside the listed range resolves to the
  RDT rate (RoW/Legends variant included) *before* the versioned and own-table lookups.
  Anything else behaves exactly as before.
- Left alone on purpose: 72 pairs where both tables hand out the same quantity (KBD /
  Runite bar, 1 each way). Nothing can tell them apart, so the own-table rate stays.
- RDT quantities are NOT global constants — some monsters roll the table twice (Abyssal
  Sire: Law rune 90) or carry a range (38–43 Rune arrow) — hence per monster, not per item.
- `VerifyResolution` covers both directions at Sarachnis, Kalphite Queen, Giant Mole and
  Demonic gorilla, plus the ambiguous KBD case. Its old checks pass quantity 0, which no
  table roll uses, i.e. "the monster's own drop".
